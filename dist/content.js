...
let currentDuration = 'Swing'; // Default
let activeCustomBalance = 0;
function injectNativeWidget(targetElement, stockName, ticker, currentPrice) {
    if (document.getElementById('ta-native-widget'))
        return;
  ...
  `;
            ...
            setTimeout(() => {
                ...
                if (!isNaN(qty) && qty > 0) {
                    ...
                    for (const input of inputs) {
                        if (input.id === 'ta-custom-balance')
                            continue;
                        if (input.closest('#ta-native-widget'))
                            continue;
                ...
                const legInputs = Array.from(document.querySelectorAll('input[class*="leg_input"]'));
                if (legInputs.length > 0) {
                    let slInput = null;
                    let targetInput = null;
                    for (const input of legInputs) {
                        const closestContainer = input.closest('div[class*="stoploss-and-target"]');
                    ...
                    if (!slInput && legInputs.length > 0)
                        slInput = legInputs[0];
                    if (!targetInput && legInputs.length > 1)
                        targetInput = legInputs[1];
                    ...
                    for (const label of labels) {
                        if (label.children.length > 0)
                            continue;
                        if (label.closest('#ta-native-widget'))
                            continue;
    ...
    const balInput = document.getElementById('ta-custom-balance');
    ...
    balInput.addEventListener('keypress', (e) => { if (e.key === 'Enter')
        handleBalChange(); });
...
function fetchAnalysisData(stockName, ticker, currentPrice, durationType, customBalance) {
    ...
    chrome.runtime.sendMessage(requestData, (response) => {
        ...
        if (response && response.success && response.data) {
            ...
            if (newsPanel && newsListEl && response.data.newsList && response.data.newsList.length > 0) {
                ...
                newsListEl.innerHTML = response.data.newsList.slice(0, 4).map((n) => `<div style="border-bottom: 1px solid #f1f2f4; padding-bottom: 8px;">
            ...
            if (loadingDiv)
                loadingDiv.innerHTML = "Error analyzing data. Check console.";
        }
    });
}
function scanPageForInjection() {
    ...
    if (hostname.includes('groww.in')) {
        ...
        if (targetElement) {
            stockName = targetElement.innerText;
        ...
        if (targetElement) {
            stockName = targetElement.innerText.split(' ')[0];
            ticker = stockName;
            injectNativeWidget(targetElement, stockName, ticker, currentPrice);
        }
    }
...
function injectIPOColumn() {
    ...
    try {
        ...
        theads.forEach(thead => {
            if (thead.querySelector('.ta-gmp-th'))
                return;
        ...
        rows.forEach(row => {
            if (row.querySelector('.ta-gmp-td'))
                return;
            const nameSpan = row.querySelector('span[aria-label="Company name"], td span.contentPrimary.truncate');
            if (!nameSpan)
                return;
...
function injectMarketDepthVisualizer() {
    if (!window.location.hostname.includes('groww.in'))
        return;
    if (document.getElementById('ta-depth-widget'))
        return;
    let marketDepthHeading = null;
    let bidTotalEl = null;
    ...
    for (let i = 0; i < els.length; i++) {
        ...
        if (node.childNodes.length === 1 && node.childNodes[0].nodeType === 3) {
            ...
            if (text === 'Market depth')
                marketDepthHeading = node;
            if (text === 'Bid Total')
                bidTotalEl = node;
        }
    }
    if (!marketDepthHeading || !bidTotalEl)
        return;
    const widget = document.createElement('div');
    ...
    setInterval(() => {
        ...
        let section = bidPriceEl;
        for (let up = 0; up < 12; up++) {
            if (!section)
                break;
            if (section.innerText && section.innerText.includes('Ask Price'))
                break;
            section = section.parentElement;
        }
        if (!section || !section.innerText.includes('Ask Price'))
            return;
...
function renderDepthChart(bids, asks, isDark) {
    ...
    if (!container || !svg || !pathBidArea || !pathBidLine || !pathAskArea || !pathAskLine || !hoverLayer || !midPriceEl)
        return;
    ...
    if (maxQty <= 0)
        return;
    const priceToX = (p) => ((p - minP) / (maxP - minP)) * width;
    const qtyToY = (q) => height - (q / maxQty) * height;
    ...
    allPts.forEach(pt => {
        ...
        rect.addEventListener('mouseout', () => {
            ...
            if (tooltip) {
                ...
                setTimeout(() => { if (tooltip.style.opacity === '0')
                    tooltip.style.display = 'none'; }, 200);
            }
        });
...
function injectPricePrediction() {
    if (!window.location.hostname.includes('groww.in'))
        return;
    if (document.getElementById('ta-pred-widget'))
        return;
    ...
    setInterval(() => {
        ...
        if (!bidPriceEl2)
            return;
        let section2 = bidPriceEl2;
        for (let up = 0; up < 12; up++) {
            if (!section2)
                break;
            if (section2.innerText && section2.innerText.includes('Ask Price'))
                break;
            section2 = section2.parentElement;
        }
        if (!section2 || !section2.innerText.includes('Ask Price'))
            return;
        const nodes2 = [];
        ...
        const upperBand = [currentPrice];
        const lowerBand = [currentPrice];
        ...
        const svg = document.getElementById('ta-pred-svg');
        ...
        if (!container || !pathLine || !pathBand || !anchor || !hoverLayer || !labelsDiv)
            return;
        ...
        const prToY = (p) => H - ((p - minY) / (maxY - minY)) * H;
        const minToX = (m) => (m / MINS) * W;
        // Draw line
        let dLine = '';
        ...
        for (let i = 1; i <= MINS; i++)
            dBand += `L ${minToX(i).toFixed(1)} ${prToY(upperBand[i]).toFixed(1)} `;
        for (let i = MINS; i >= 0; i--)
            dBand += `L ${minToX(i).toFixed(1)} ${prToY(lowerBand[i]).toFixed(1)} `;
        dBand += 'Z';
        ...
        anchor.setAttribute('x1', '0');
        anchor.setAttribute('y1', anchorY);
        anchor.setAttribute('x2', W.toString());
        anchor.setAttribute('y2', anchorY);
        ...
        predictedPrices.forEach((price, m) => {
            ...
            hitRect.addEventListener('mouseout', () => {
                ...
                if (tt) {
                    tt.style.opacity = '0';
                    setTimeout(() => { if (tt.style.opacity === '0')
                        tt.style.display = 'none'; }, 200);
                }
            });
...
function injectMutualFundProjector() {
    ...
    if (currMatch)
        pageCv = parseFloat(currMatch[1].replace(/,/g, ''));
    let pageXirr = null;
    const xirrMatch = bodyText.match(/XIRR(?:[\s\nv^]|\(Active\))*([+\-0-9.]+)%/i);
    if (xirrMatch)
        pageXirr = parseFloat(xirrMatch[1]);
    ...
    const headings = Array.from(document.querySelectorAll('div, h1, h2, span, p')).filter(el => {
        return el.innerText && el.innerText.match(/^Investments\s*\([0-9]+\)$/i);
    });
    ...
    if (invMatch)
        investedValue = parseFloat(invMatch[1].replace(/,/g, ''));
    let sipValue = 0;
    ...
    const sipInput = document.getElementById('ta-mf-sip-input');
    ...
    const updateMfChart = () => {
        ...
        const years = parseInt(document.getElementById('ta-mf-years').value) || 10;
        const yearValLabel = document.getElementById('ta-mf-year-val');
        if (yearValLabel)
            yearValLabel.innerText = `${years} Years`;
        const endLabel = document.getElementById('ta-mf-chart-end-label');
        if (endLabel)
            endLabel.innerText = `Year ${years}`;
        let r = rate / 100 / 12;
        if (r === 0)
            r = 0.000001;
        ...
        if (!container || !svg || !line || !area || !hoverLayer)
            return;
        const width = container.clientWidth || 300;
    ...
    const yearSlider = document.getElementById('ta-mf-years');
    if (yearSlider)
        yearSlider.addEventListener('input', updateMfChart);
...
function parsePositions() {
    ...
    for (const row of allRows) {
        const hRow = row;
        if (hRow.closest('#ta-positions-analyzer'))
            continue;
        ...
        if (ignoreList.some(kw => lowerCo.includes(kw)))
            continue;
        const ticker = href
            ? href.replace('/stocks/', '').split('-')[0].toUpperCase()
            : company.split(' ')[0].toUpperCase();
        const parseRupee = (s) => parseFloat(s.replace(/[^0-9.]/g, ''));
...
let autoExitInterval = null;
let hasAutoExitedToday = false;
function detectDurationType() {
    const bodyText = document.body.innerText;
    if (bodyText.match(/Equity Intraday/i))
        return 'Intraday';
    return 'Swing';
}
function injectPositionsAnalyzer() {
    ...
    const headerEl = widget.querySelector('div[style*="sticky"]');
    if (headerEl) {
        ...
        autoExitCb._updateVisual = updateVisual;
    }
    ...
    autoExitInterval = setInterval(() => {
        const _cb = document.getElementById('ta-auto-exit');
        if (!_cb || !_cb.checked || hasAutoExitedToday)
            return;
        const now = new Date();
...
function fetchAndRenderPositions() {
    ...
    chrome.runtime.sendMessage(req, (response) => {
        ...
        if (!response || !response.success || !response.data) {
            if (content)
                content.innerHTML = `<div style="text-align:center;color:#9aa0a6;padding:24px;font-size:13px;">Analysis unavailable. Retrying shortly...</div>`;
            return;
        }
        renderCards(content, response, activePositions, durationType);
    });
}
function updateBanner(realisedPnl, unrealisedPnl, hitRate) {
...
function renderCards(content, response, activePositions, durationType) {
    if (!content)
        return;
    let html = '';
    ...
    for (const pos of response.data.positions) {
        ...
        if (pos.recommendation === 'Hold') {
            recColor = '#00b386';
            recBg = '#e8f9f4';
        }
        else if (pos.recommendation === 'Add More') {
            recColor = '#3b82f6';
            recBg = '#eff6ff';
        }
        else if (pos.recommendation === 'Exit All') {
            recColor = '#eb5b3c';
            recBg = '#fef2f0';
        }
        else if (pos.recommendation === 'Book Partial') {
            recColor = '#f59e0b';
            recBg = '#fffbeb';
        }
        let minigraph = '';
        if (pos.projectedPrices && pos.projectedPrices.length > 0) {
            ...
            pos.projectedPrices.forEach((p, idx) => {
